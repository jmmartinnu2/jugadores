...
with col1:
    if photos.exists():
        st.image(str(photos), caption=selected, use_column_width=True)
    else:
        st.warning("Imagen de jugador no encontrada.")
    if clubs.exists():
        st.image(str(clubs), width=100, caption="Escudo del club")
    else:
